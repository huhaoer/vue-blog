...
// 分页博客数据
Mock.mock(/^\/api\/blogs(\?.+)?&/, 'get', function (opts) {
    const index = opts.url.indexOf('?')
    let s;
    if (index !== -1) {
        s = opts.url.slice(index + 1)
    } else {
        s = ''
    }
    ...
    return Mock.mock({
        ...
        data: {
            ...
            [`rows|${query.limit || 10}`]: [
                {
                    id: '@guid', // 使用全局生成uid随机字符串
                    title: '@ctitle(1,50)', // 使用随机生成中文标题,1到50个字
                    desc: '@cparagraph(1,10)', // 使用随机生成中文段落,1到10哥句子
                    category: {
                        "id|1-10": 0,
                        name: '分类@id'
                    },
                    ...
                    "thumb|1": [Mock.Random.image('300', '#000', '#f40', 'random image'), null], // 使用Mock函数随机生成图片,可以有图片也可以为null
                    createDate: `@date('T')`,
                }
            ]
            // total: 0,
...
Mock.mock('/api/blogTypes', 'get', {
    code: 0,
    msg: '',
    'data|20-30': [ // 模拟10-20条数据
        {
            "id|+1": 1, // id自增1
            name: '分类@id', // 使用当前id字段为变量进行拼接
            "count|0-300": 0,
            "order|+1": 0
        }
    ]
